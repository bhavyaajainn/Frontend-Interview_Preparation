...
allTests.push(new Promise(resolve => {
  ...
  const hasAll = result.includes('blog=learnersbucket') && result.includes('name=prashant');
  console.log(`${hasAll ? 'PASS' : 'FAIL'}: TC2: both cookies visible before expiry`);
  resolve();
}));
...
allTests.push(new Promise(resolve => {
  ...
  setTimeout(() => {
    ...
    console.log(`${expired && persisted ? 'PASS' : 'FAIL'}: TC3: expired cookie removed, non-expiring persists`);
    resolve();
  }, 1500);
}));
...
allTests.push(new Promise(resolve => {
  ...
  setTimeout(() => {
    const result = document.myCookie;
    const ok = result.includes('a=1') && result.includes('c=3') && !result.includes('b=2');
    console.log(`${ok ? 'PASS' : 'FAIL'}: TC4: only b=2 expired, a and c survive`);
    resolve();
  }, 1500);
}));
