...
// TC1 — pushWithEvent fires the 'add' listener with the pushed items
{
  const arr = [];
  let received;
  arr.addListener('add', (_, items) => { received = items; });
  arr.pushWithEvent('add', [1, 2, 3]);
  test('TC1: pushWithEvent triggers add listener with items', received, [1, 2, 3]);
}
...
// TC3 — popWithEvent fires the 'remove' listener with the popped element
{
  const arr = [10, 20, 30];
  let removed;
  arr.addListener('remove', (_, item) => { removed = item; });
