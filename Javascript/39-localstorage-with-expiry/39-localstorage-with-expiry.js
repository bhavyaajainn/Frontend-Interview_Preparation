...
allTests.push(
  new Promise((resolve) => {
    ...
    setTimeout(() => {
      test(
        "TC2: returns null after expiry",
        myLocalStorage.getItem("tc2"),
        null,
      );
      resolve();
    }, 300);
...
allTests.push(
  new Promise((resolve) => {
    test(
      "TC3: non-existent key returns null",
      myLocalStorage.getItem("__missing__"),
      null,
    );
    resolve();
  }),
...
allTests.push(
  new Promise((resolve) => {
    ...
    setTimeout(() => {
      test(
        "TC4: default maxAge keeps item alive",
        myLocalStorage.getItem("tc4"),
        "persistent",
      );
      resolve();
    }, 100);
...
allTests.push(
  new Promise((resolve) => {
    ...
    setTimeout(() => {
      test(
        "TC5: overwritten key expires with new maxAge",
        myLocalStorage.getItem("tc5"),
        null,
      );
      resolve();
    }, 300);
...
allTests.push(
  new Promise((resolve) => {
    ...
    test(
      "TC7: object value stored and retrieved correctly",
