...
function deepFreeze(obj) {
  // 1. Get all own property names of obj (Object.getOwnPropertyNames)
  // 2. For each property, if the value is a non-null object, recursively deepFreeze it
  // 3. Call Object.freeze(obj) to freeze the current level, then return obj
...
function update(inputObj, action) {
  // TODO
  //1. find the action type- this will be done using deep freeze
  //2. Write the action definitions
  if ("_push_" in action) {
    return deepFreeze([...inputObj, ...action._push_]);
  }
  if ("_replace_" in action) {
    return deepFreeze(action._replace_);
  }
  if ("_merge_" in action) {
    return deepFreeze({ ...inputObj, ...action._merge_ });
  }
  if ("_transform_" in action) {
    return deepFreeze(action._transform_(inputObj));
  }
  const isArray = Array.isArray(inputObj);
  const result = isArray ? [...inputObj] : { ...inputObj };
  for (const key of Object.keys(action)) {
    result[key] = update(inputObj[key], action[key]);
  }
